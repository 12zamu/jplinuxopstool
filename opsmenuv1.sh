...
check_host()
{
  nc -zw5 $remote_hostname 22 > /dev/null
  check=`echo $?`
  return
}
...
remote()
{
  ...
  then
    ...
    echo "Hostname invalid or port 22 is not open"
  fi
}
...
check_rhmem()
{
  #remote " free -m ; echo "###Top_5_memory_eating_process###" ;  ps auxf | sort -nr -k 4 | head -5"
  remote "free -m"
  echo ""
